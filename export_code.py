...
if __name__ == '__main__':
    exclude_scripts = ['export_code.py', 'export_html.py']  # Replace with your actual script names
    directory_to_search = '.'
    extensions = ('.py', '.json', '.xml', '.yaml')
    #exclude_directories = {'venv', '.idea', '__pycache__', '.git'}
    exclude_directories = {'venv', '.idea', '__pycache__', '.git', 'testing_server'}

    # Listing the files
    files = list_files(directory_to_search, extensions, exclude_directories, exclude_scripts)

    # Writing the file contents to the output txt file
    output_txt_file = 'collected_files_content.txt'
