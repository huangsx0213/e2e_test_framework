    ...
    def _read_test_cases(self):
        ...
        for _, row in self.test_cases_df.iterrows():
            ...
            test_step = {
                ...
                "Defaults": row.get('Defaults', "").strip(),
                "Body Modifications": row.get('Body Modifications').strip() if row.get('Body Modifications') else '{}',
                "Run": row.get('Run', "").strip(),
                "Tags": row.get('Tags', "").strip(),
                "Exp Status": row.get('Exp Status', ""),
                "Exp Result": row.get('Exp Result', "").strip(),
                "Save Fields": row.get('Save Fields', "").strip(),
