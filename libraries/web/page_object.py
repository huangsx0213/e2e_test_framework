...
class PageObject:
    ...
    def _load_page_modules(self):
        ...
        for _, row in self.page_object_df.iterrows():
            ...
            if page_name not in modules:
                modules[page_name] = {}
            if module_name not in modules[page_name]:
                modules[page_name][module_name] = []

            modules[page_name][module_name].append({
    ...
    def execute_module(self, page_name, module_name, parameters=None):
        ...
        for action_info in module_actions:
            ...
            if action == 'open_url':
                self.web_actions.open_url(parameters[param_names[0]])
            elif action == 'send_keys':
                self.web_actions.send_keys(element, parameters[param_names[0]])
            elif action == 'click':
                self.web_actions.click(element)
            elif action == 'clear':
                self.web_actions.clear(element)
            elif action == 'select_by_value':
                self.web_actions.select_by_value(element, parameters[param_names[0]])
            elif action == 'select_by_visible_text':
                self.web_actions.select_by_visible_text(element, parameters[param_names[0]])
            elif action == 'select_by_index':
                self.web_actions.select_by_index(element, parameters[param_names[0]])
            elif action == 'hover':
                ...
                return self.web_actions.get_attribute(element, parameters[param_names[0]])
                ...
                return self.web_actions.element_text_should_be(element, parameters[param_names[0]])
            elif action == 'element_text_should_contains':
                return self.web_actions.element_text_should_contains(element, parameters[param_names[0]])
            elif action == 'title_should_be':
                return self.web_actions.title_should_be(parameters[param_names[0]])
            elif action == 'title_should_contains':
                return self.web_actions.title_should_contains(parameters[param_names[0]])
            elif action == 'wait_for_text_to_be_present':
                return self.web_actions.wait_for_text_to_be_present(locator, parameters[param_names[0]])
                ...
                return self.web_actions.execute_script(parameters[param_names[0]], element)
